...
def _env_bool(name, default=False):
    ...
    if value is None:
        return default
    return value.lower().strip() in {"1", "true", "yes", "si"}


APP_NAME = "EVA"
APP_VERSION = "0.4.0"
HOST = os.getenv("EVA_HOST", "127.0.0.1")
PORT = int(os.getenv("EVA_PORT", "5000"))
DEBUG = _env_bool("EVA_DEBUG", True)
...
EMBEDDINGS_ENABLED = _env_bool("EVA_EMBEDDINGS_ENABLED", True)
...
ARCHIVIST_ENABLED = _env_bool("EVA_ARCHIVIST_ENABLED", True)
...
HEARTBEAT_ENABLED = _env_bool("EVA_HEARTBEAT_ENABLED", False)
HEARTBEAT_INTERVAL = int(os.getenv("EVA_HEARTBEAT_INTERVAL", "60"))
AUTONOMY_ENABLED = _env_bool("EVA_AUTONOMY_ENABLED", False)
